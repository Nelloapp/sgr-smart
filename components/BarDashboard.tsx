...
import { Clock, Wine, CheckCircle, Send, AlertCircle } from 'lucide-react-native';
...
export const BarDashboard: React.FC<BarDashboardProps> = ({ 
  orders, 
  onUpdateStatus,
  refreshTrigger
}) => {
  ...
  const pendingOrders = orders.filter(order => order.drinkStatus === 'pending');
  const servedOrders = orders.filter(order => order.drinkStatus === 'served');
  ...
  const renderOrderCard = (order: Order) => {
    ...
    const pendingDrinkItems = drinkItems.filter(item => item.status === 'pending');
    ...
    return (
      <View key={order.id} style={styles.orderCard}>
        ...
        {pendingDrinkItems.length === 0 && order.drinkStatus !== 'served' && (
          <TouchableOpacity
            style={[styles.actionButton, { backgroundColor: colors.itemServed }]}
