...
import { Clock, Wine, CheckCircle, Send, AlertCircle, ChevronDown, ChevronUp } from 'lucide-react-native';
...
export const BarOrdersList: React.FC<BarOrdersListProps> = ({ 
  orders, 
  onUpdateStatus 
}) => {
  ...
  const hasNewPendingItems = (order: Order): boolean => {
    return order.items.some(item => 
      item.type === 'drink' && item.status === 'pending'
    );
  };
  ...
  const renderOrderItem = ({ item }: { item: Order }) => {
    // Filtra solo gli elementi di bevande
    const drinkItems = item.items.filter(orderItem => orderItem.type === 'drink');
    // Filtra gli elementi pendenti
    const pendingDrinkItems = drinkItems.filter(orderItem => orderItem.status === 'pending');
    ...
    return (
      <View style={[
        styles.orderCard, 
        { borderLeftColor: getStatusColor(item.drinkStatus) },
        ...
        >
          <View style={styles.orderInfo}>
            ...
            <View style={styles.statusContainer}>
              {getStatusIcon(item.drinkStatus)}
              <Text style={[styles.statusText, { color: getStatusColor(item.drinkStatus) }]}>
                {getStatusText(item.drinkStatus)}
              </Text>
            </View>
          </View>
        ...
        {isExpanded && pendingDrinkItems.length > 0 && (
          <View style={styles.pendingActionsContainer}>
            <Text style={styles.pendingCountText}>
              {pendingDrinkItems.length} {pendingDrinkItems.length === 1 ? 'elemento' : 'elementi'} da preparare
            </Text>
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                ...
                Alert.alert(
                  "Segna come serviti",
                  `Vuoi segnare tutti i ${pendingDrinkItems.length} elementi come serviti?`,
                  [
                    ...
                    { 
                      ...
                      onPress: () => {
                        ...
                        pendingDrinkItems.forEach(orderItem => {
        ...
        {isExpanded && pendingDrinkItems.length === 0 && item.drinkStatus !== 'served' && (
          <View style={styles.orderFooter}>
            <Text style={styles.totalText}>
              Totale Bevande: {formatCurrency(drinkItems.reduce((sum, item) => sum + (item.price * item.quantity), 0))}
            </Text>
            <TouchableOpacity
              style={[styles.actionButton, { backgroundColor: colors.itemServed }]}
